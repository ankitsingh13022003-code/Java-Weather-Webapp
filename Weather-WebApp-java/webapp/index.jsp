...
<body>
   ...
   <script>
    ...
    // Normalize incoming condition text
    var val = document.getElementById("wc").value.trim().toLowerCase();

    // CLOUDS (covers: Clouds, Cloudy, Overcast, Partly cloudy, Mostly cloudy)
    ...
    // CLEAR, SUNNY
    else if (val.includes("clear") || val.includes("sunny")) {
    ...
    // RAIN (covers: Rain, Light rain, Patchy rain, Heavy rain)
    ...
    // MIST (covers: Mist, Fog)
    else if (val.includes("mist") || val.includes("fog")) {
    ...
    // SNOW (covers: Snow, Light Snow, Blizzard)
    else if (val.includes("snow") || val.includes("blizzard")) {
    ...
    // HAZE
    else if (val.includes("haze") || val.includes("smoke")) {
